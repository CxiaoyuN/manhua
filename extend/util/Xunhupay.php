...
class Xunhupay
{
    public static function http_post($url,$data){
        if(!function_exists('curl_init')){
            throw new \think\Exception('php未安装curl组件',500);
        }
        ...
        if($httpStatusCode!=200){
            throw new \think\Exception("invalid httpstatus:{$httpStatusCode} ,response:$response,detail_error:".$error,$httpStatusCode);
        }

        return $response;
    }

    protected static function isWebApp(){
    ...
    protected static  function generate_xh_hash(array $datas,$hashkey){
        ksort($datas);
        reset($datas);
    ...
    protected static  function is_wechat_app(){
