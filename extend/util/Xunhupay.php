...
class Xunhupay
{
    public function submit($order_id, $money, $pay_type = 'alipay')
    {
        ...
        $data = array(
            ...
            'plugins' => 'racooncms',//必须的，根据自己需要自定义插件ID，唯一的，匹配[a-zA-Z\d\-_]+
            'appid' => $appid, //必须的，APPID
            'trade_order_id' => $order_id, //必须的，网站订单ID，唯一的，匹配[a-zA-Z\d\-_]+
            'payment' => $pay_type,//必须的，支付接口标识：wechat(微信接口)|alipay(支付宝接口)
            'total_fee' => $money,//人民币，单位精确到分(测试账户只支持0.1元内付款)
            'title' => config('site.site_name') . '充值', //必须的，订单标题，长度32或以内
            'time' => time(),//必须的，当前时间戳，根据此字段判断订单请求是否已超时，防止第三方攻击服务器
            'notify_url' => config('site.url') . '/xunhunotify', //必须的，支付成功异步回调接口
            'return_url' => config('site.url') . '/feedback',//必须的，支付成功后的跳转地址
            'callback_url' => config('site.url') . '/charge',//必须的，支付发起地址（未支付或支付失败，系统会会跳到这个地址让用户修改支付信息）
    ...
    public static function http_post($url, $data)
    {
        ...
        if ($httpStatusCode != 200) {
            return"invalid httpstatus:{$httpStatusCode}";
        }
